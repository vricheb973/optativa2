...
public class PedidoMapper {
	...
	public static PedidoDTO toDTO(Pedido pedido) {
		...
		List<DireccionDTO> direcciones = pedido.getCliente().getDirecciones().stream()
				.filter(d -> d.getActiva())
				.map(d -> {
					DireccionDTO dirDTO = new DireccionDTO();
					dirDTO.setCalle(d.getCalle());
					dirDTO.setNumero(d.getNumero());
					dirDTO.setPoblacion(d.getPoblacion());
					dirDTO.setId(d.getId());
					dirDTO.setActiva(d.getActiva());
		...
		dto.setDireccion(direcciones.isEmpty()?"Sin dirección":direcciones.get(0).getCalle()+" "+direcciones.get(0).getNumero()+" "+direcciones.get(0).getPoblacion());
